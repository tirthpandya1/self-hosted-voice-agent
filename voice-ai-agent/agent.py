...
import json
import torch
import time # For Strategy 3 if needed
import soundfile as sf # For saving debug TTS
...
DESCRIPTION_HI = "Divya speaks in a clear, moderate-paced and soothing voice with an Indian Hindi accent. The recording is of very high quality with no background noise."
...
DESCRIPTION_DEFAULT = DESCRIPTION_EN # Default to English if language is not hi

OLLAMA_MODEL = "llama3.2:3b" # Example, make sure this model is pulled in Ollama. Changed from llama3.2:3b
...
tts_device = "cuda:0" if torch.cuda.is_available() else "cpu" # Reverted to GPU, assuming Parler works on it now
...
try:
    print(f"Loading Indic Parler TTS model ({PARLER_MODEL_NAME}) to device: {tts_device} ... This may take a while.")
...
def transcribe_audio_whisper_cpp(audio_path): # Using the more robust version from previous steps
    ...
    try:
        ...
        elif isinstance(transcription_json, list):
        ...
        elif isinstance(lang_data_from_json, str):
            detected_language = lang_data_from_json
        
        if detected_language == "unknown": # Fallback to stderr if JSON doesn't have detailed lang info
             for line in process.stderr.splitlines(): 
                if "auto-detected language:" in line:
...
def query_llm(user_query, rag_context, input_lang="en"):
    print(f"Querying LLM ({OLLAMA_MODEL}) with input_lang hint: {input_lang}...")
    system_prompt = ("You are a helpful multilingual AI assistant. User might speak English, Hindi, or Hinglish. Use context. Be concise.")
...
def synthesize_speech_indic_parler(text_to_speak, lang_code="en"):
    # print(f"DEBUG TTS INPUT: Text='{text_to_speak}', Lang Code for Desc='{lang_code}'") 
    if not parler_model:
        print("Indic Parler TTS model not loaded. Cannot synthesize.")
        return None, 24000  
    print(f"Synthesizing with Indic Parler (Device: {tts_device}): lang_code for desc: '{lang_code}', text: '{text_to_speak[:60]}...'")
    description = DESCRIPTION_DEFAULT
    if lang_code == "hi": description = DESCRIPTION_HI
    elif lang_code == "en": description = DESCRIPTION_EN
    
    audio_arr_int16 = None 
    tts_sample_rate = parler_model.config.sampling_rate if parler_model else 24000
    audio_bytes = None
    ...
    try:
        ...
        with torch.no_grad():
            generation = parler_model.generate(
                ...
                do_sample=True, temperature=0.6, top_k=50, top_p=0.95, max_length=1000).cpu()
        
        print(f"DEBUG TTS: Raw 'generation' tensor shape: {generation.shape}, dtype: {generation.dtype}, min: {generation.min().item() if generation.numel()>0 else 'N/A'}, max: {generation.max().item() if generation.numel()>0 else 'N/A'}")
        audio_arr = generation.numpy().squeeze()
        if audio_arr.ndim > 1: audio_arr = audio_arr.flatten()

        if audio_arr.size == 0: 
            print("DEBUG TTS: audio_arr is empty after numpy conversion. No audio data.")
        ...
        print(f"DEBUG TTS: audio_arr_int16 shape: {audio_arr_int16.shape}, dtype: {audio_arr_int16.dtype}, min: {audio_arr_int16.min()}, max: {audio_arr_int16.max()}")
        audio_bytes = audio_arr_int16.tobytes()
        tts_sample_rate = parler_model.config.sampling_rate
        
        if audio_arr_int16 is not None and audio_arr_int16.size > 0 :
            ...
            # print(f"Saving debug TTS audio to: {debug_audio_path} (SR: {tts_sample_rate})") # Less verbose
            sf.write(debug_audio_path, audio_arr_int16, samplerate=tts_sample_rate, subtype='PCM_16')
            print(f"DEBUG TTS: Debug audio saved to {debug_audio_path}")
        else:
            print("DEBUG TTS: No valid audio array to save for debugging.")

        # print(f"Indic Parler TTS synthesis successful. Sample rate: {tts_sample_rate}") # Less verbose
...
def play_audio_bytes_simpleaudio(audio_bytes, sample_rate, sample_width=2, channels=CHANNELS):
    if not audio_bytes or len(audio_bytes) == 0:
        print("No audio data to play (simpleaudio).")
        return
    ...
    try:
        ...
        play_obj.wait_done() # <<< SEGFAULT LIKELY AROUND OR AFTER THIS
...
def play_audio_bytes_sounddevice(audio_bytes, sample_rate, channels=CHANNELS):
    if not audio_bytes or len(audio_bytes) == 0:
        print("No audio data to play (sounddevice).")
        return
    # print(f"Playing with sounddevice: {len(audio_bytes)} bytes, SR: {sample_rate} Hz")
    try:
        audio_np = np.frombuffer(audio_bytes, dtype=np.int16)
        if channels > 1 and audio_np.size % channels != 0 :
             print(f"Warning (sounddevice): attempting to reshape for {channels} channels")
             audio_np = audio_np.reshape(-1, channels)
        
        if audio_np.size > 0:
            print(f"sounddevice: Playing audio with {len(audio_bytes)} bytes, SR: {sample_rate} Hz")
            sd.play(audio_np, samplerate=sample_rate, blocking=True, latency='low')
            print("Playback with sounddevice finished.")
        else:
            print("Error (sounddevice): audio_np array is empty after processing.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not os.path.exists("audio_files"):
        os.makedirs("audio_files"); print("Created directory: audio_files")

    playback_library = "sounddevice"  # OPTIONS: "simpleaudio" or "sounddevice"
    print(f"Using playback library: {playback_library}")
    ...
    try:
        while True:
            ...
            if not user_query_text:
                ...
                if err_audio_bytes:
                    if playback_library == "sounddevice": play_audio_bytes_sounddevice(err_audio_bytes, err_sr)
                    else: play_audio_bytes_simpleaudio(err_audio_bytes, err_sr)
                continue
            
            primary_lang_code_for_llm = detected_lang_whisper 
            ...
            if audio_output_bytes:
                ...
                if fb_audio:
                    if playback_library == "sounddevice": play_audio_bytes_sounddevice(fb_audio, fb_sr)
                    else: play_audio_bytes_simpleaudio(fb_audio, fb_sr)
