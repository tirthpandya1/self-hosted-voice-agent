...
def transcribe_audio_whisper_cpp(audio_path):
    ...
    try:
        ...
        # Language detection from the JSON structure
        # It might be under a top-level "language" key or "detected_language" object
        if "language" in transcription_json:
            lang_obj = transcription_json["language"]
            if isinstance(lang_obj, str): # e.g. "language": "en"
                detected_language = lang_obj
            elif isinstance(lang_obj, dict): # e.g. "language": {"language": "en", "probability": 0.9}
                detected_language = lang_obj.get("language", lang_obj.get("lang", "unknown"))
        elif "detected_language" in transcription_json: # another common key
             lang_obj = transcription_json["detected_language"]
             if isinstance(lang_obj, str):
                detected_language = lang_obj
             elif isinstance(lang_obj, dict):
                detected_language = lang_obj.get("language", lang_obj.get("lang", "unknown"))
        
        # If still unknown, try to get from the first segment (if available)
        if detected_language == "unknown" and segments and isinstance(segments, list) and "language" in segments[0]:
             detected_language = segments[0].get("language", {}).get("lang", "unknown")


        print(f"Transcription from JSON: {full_text}")
        print(f"Detected language from JSON (by Whisper): {detected_language}")
